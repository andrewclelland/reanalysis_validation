...
grcf.GRIB1_LOCAL_TO_CF[grcf.G1LocalParam(1, 128, 98, 260048)] = grcf.CFName('lwe_precipitation_rate', None, 'kg m-2 s-1')
...
def process_era5_hourly_from_grib(var_name, level, area, duration, start_year, end_year, start_month, end_month):
    ...
    if duration == 'hourly':
    ...
    elif duration in ['6-hourly', 'daily']:
        # aggregate the data
        time_coord = hourly_cube.coord(axis='t')
        ...
        if duration == '6-hourly':
            icc.add_categorised_coord(hourly_cube, '6-hourly_period', time_coord,
                                      lambda coord, value: time_coord.units.num2date(value).hour // 6)
            agg_coords.append('6-hourly_period')
            required_duration = 6
...
if __name__ == '__main__':
    ...
    parser.add_argument("--duration", choices=["hourly", "6-hourly", "daily"], default="hourly")
